...
void main() {
    ...
    vec3 newPosition2 = vec3(position.x, position.y , position.z) + normal * 6. * snoise(vec3(2.*uv.x,0.5*uv.y,t)) *sin(10.*uv.x + 5.*t); /// (10.*t*uv.x);

    
	float dx = P.x/100.0 + 0.5;
	float dy = P.y/100.0 + 0.5;
	float height = sin(0.03*P.x + 1.0);
	height += 0.9 * (snoise(0.01*vec2(1.2*P.x,0.8*P.y)) -0.0);
	height = smoothstep(0.0,1.0,height);
	//height += (height > 0.7) ? 0.2 * snoise(31.11*vec2(1.9*P.x,1.9*P.y)) : 0.0;
    // float height = snoise(vec2(P.x,P.y));
    // float height = dx;
	// height *= height*4*zcomp(P);
	// height += 2. * snoise(0.07*P) -0.5; 
	...
	height *= 20.;
	// height += 10.;
    vec3 newPosition3 = position + normal * height;
